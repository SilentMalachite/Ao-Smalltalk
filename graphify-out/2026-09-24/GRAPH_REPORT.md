...
Cohesion: 0.11
Nodes (59): Digits, __int128, add(), addBig(), asInt64IfFits(), Big, d, neg (+51 more)

### Community 4 - "TEST"
Cohesion: 0.06
Nodes (52): FailedProbeKeepsCurrentSession, ImageSave, ImageSaveLoad, iterator, KernelMethodsStayNative, LoadSizesOldFromHeader, NurseryGcAfterLoad, OnePlusTwoMatches (+44 more)

### Community 5 - "Scanner.cpp"
Cohesion: 0.06
Nodes (60): ArrayAndByteArrayHeaders, AssignVariantsAndComment, cmath, CommaIsABinaryCharacter, uint32_t, Scanner, i_, lexBinary (+52 more)
...
Cohesion: 0.06
Nodes (87): [Medium] ストリームの `contents` が String と Array 以外のコレクションで壊れた値を返す, [Medium] 固定長の String に書く WriteStream が、多バイト文字を黙って捨てる, Task 2: 再結合に必要なヒープとレジストリの口, リスク, Task 2: Transcript のクラス側転送とフック, WellKnown, installArray(), WellKnown (+79 more)
...
Cohesion: 0.08
Nodes (47): charconv, allocate, allocateTenured, fitsOld, growOld, header, initObject, largeObjectBytes (+39 more)

### Community 12 - "TEST"
Cohesion: 0.05
Nodes (45): ByteObjectPayloadIsNotScannedAsOops, CollectOldOnlyAfterThreshold, CompactionDuringScavengeDoesNotCorruptSlots, DeadOldSlotIsNotANurseryRoot, DestJumpPastPinDoesNotOverlap, DuplicateRootForwardedOnce, GcOld, GcRoots (+37 more)
...
### Community 19 - "Heap"
Cohesion: 0.04
Nodes (53): [Critical] 非クラス名で ao_accept_method を呼ぶと範囲外書き込み, Heap, checkNotPoisoned, containsNurseryFrom, containsNurseryTo, flipNursery, fromBump_, fromEnd_ (+45 more)

### Community 20 - "Roots"
Cohesion: 0.05
Nodes (44): Root, slot, FrameBlock, capacity, slots, used, size_t, StackWalker (+36 more)
...
### Community 24 - "Boot"
Cohesion: 0.05
Nodes (42): HandAssembledLitVarRoundTrip, HandAssembledRemoteTempRoundTrip, JumpOnNonBooleanAborts, JumpOnNonBooleanUsesMustBeBooleanAnswer, OpcodePastLastOpFails, PushNewArrayIsNilFilledArray, string, evalExpr() (+34 more)
...
Cohesion: 0.06
Nodes (87): Image, load, save, blankOut(), browserClassAt(), browserClassCount(), browserClassDefinition(), browserProtocolAt() (+79 more)
...
Cohesion: 0.13
Nodes (21): P3 — Native Dispatch, doesNotUnderstand:, method lookup / send, MethodDictionary, class hierarchy, doesNotUnderstand:, lookup, MethodDictionary (+13 more)
...
Cohesion: 0.09
Nodes (30): CLI, Global Constraints, globals 節, OOP エンコード, P7 .aoimage Implementation Plan, save, Task 3: save, Task 4: load と受け入れ評価 (+22 more)
...
Cohesion: 0.11
Nodes (33): algorithm, ao_abi, Bootstrap, BoxFromImageNativeCodeNil, BoxLiteralArrayPseudoObjects, chrono, Chunk, compile (+25 more)
...
### Community 37 - "string"
Cohesion: 0.06
Nodes (26): cassert, string, string_view, cstddef, cstdint, Heap, Oop, P3-01 Symbol intern (+18 more)

### Community 38 - "applyClassDef"
Cohesion: 0.21
Nodes (16): Task 3: rebind と上書き禁止, FileInError, error, file, method, string, string_view, isVendorStub() (+8 more)
...
Cohesion: 0.09
Nodes (40): ImageFormat, encodeNonHeap, kImageEndianLittle, kImageFillerBytes, kImageHeaderBytes, kImagePointerBits, kImageVersion, writeFiller (+32 more)
...
Cohesion: 0.06
Nodes (39): AbiSmoke, BootAndShutdownReturnZero, BootVersionShutdown, CleanupFailureReleasesItsReasonHandle, DefaultDoesNotUnderstandAborts, B2 `to:do:` bench, P4 microbench, P6 interpreter bench (+31 more)

### Community 50 - ".nil"
Cohesion: 0.08
Nodes (63): ClassRedefinitionDropsOldClassEntries, CompiledMethod accessors (P5), FlushSelectorDropsEveryClassAndKeepsOtherSelectors, InlinedToDoMillion, InstallMissingAddsAbsentAndKeepsPresent, InstallMissingReachesCachedSend, KernelBench, KernelInstall (+55 more)
...
### Community 58 - ".isHeap"
Cohesion: 0.09
Nodes (47): [Low] `Roots` が同じスロットの二重登録を許し、`collectOld` の更新は冪等でない, InlineCache, cachedClass, cachedMethod, abortDoesNotUnderstand(), abortEvaluation(), abortFailedSend(), abortReasonText() (+39 more)
...
Cohesion: 0.11
Nodes (51): cctype, [Medium] メソッド辞書の拡張に失敗するとメソッドを黙って捨て、`installMethod` は成功を返す, [High] チャンク経由でメソッドを再定義してもメソッドキャッシュを無効化しない, accept, Task 10: accept, acceptClassSource(), acceptMethodSource(), anyMethodIn() (+43 more)

### Community 65 - "allocateRetry"
Cohesion: 0.05
Nodes (55): [Critical] スキャベンジ中に `collectOld` が走ると、移動前のヘッダ位置へ書き込む, [High] old 4MB 固定と promote-all のため、生存約 2.5MB で毎回無駄なフル GC を回して実質停止する, [High] old 枯渇でスキャベンジが失敗すると、転送済みオブジェクトがナーサリに残り、同一性が割れる, [High] ナーサリ半面（1MB）を超えるオブジェクトは一切割り当てられず、失敗も見えない, [High] ブートストラップしたクラスの名前が Symbol ではなくクラス nil のバイト列で、`Object name` にメッセージを送れない, [Low] identity hash が 16 ビットで、SPEC のサイドテーブルがない, [Medium] グローバル `Smalltalk` がクラス nil の 57 要素固定配列で、どのメッセージも通らない, 指摘 (+47 more)
...
Cohesion: 0.22
Nodes (16): CompileError, message, span, string, [Critical] クラス定義でない文字列が AO_OK で黙って捨てられる, [High] vendor file-in のメソッドコンパイル失敗を成功として返し、17 メソッドが黙って欠落する, Task 5: `ao filein` と評価, applyChunks() (+8 more)
...
Cohesion: 0.10
Nodes (35): AoTranscriptFn, CountsAnswerMinusOneOnFailure, 04 Object・Behavior・コレクション系 Kernel ネイティブ, テストの穴, 既報との関係（件数に含めない）, 概要, [Low] count 系 ABI がエラーで AO_ERR(=1) を返し、件数 1 と区別できない, B3 失敗の伝搬とキャッシュの無効化（テーマ 2） (+27 more)
...
Cohesion: 0.11
Nodes (18): Global Constraints, P6b vendor file-in Implementation Plan, Self-review, Task 1: changes 切り出し, Task 2: ピンと vendor ソース, Task 4: LOAD_ORDER とホストパッチ, Task 6: フェーズ完了, 仕様 (+10 more)

### Community 114 - "ImageLoad.cpp"
Cohesion: 0.16
Nodes (35): decodeNonHeap, readHeader, acceptWord(), bindAll(), checkGlobals(), byte, size_t, string (+27 more)

### Community 115 - ".build"
Cohesion: 0.25
Nodes (8): Actions, MainMenu, MenuAction, NSMenu, NSMenuItem, String, Void, Selector

### Community 116 - "vector"
Cohesion: 0.26
Nodes (11): vector, isClassShaped(), superclassOf(), pair, size_t, holdsBinding(), liveClasses(), methodHoldingDropped() (+3 more)
...
### Community 123 - ".heapPointer"
Cohesion: 0.29
Nodes (6): clearWeakAfterOldMark, Gc::collectOld(), Gc::copy(), Heap::checkNotPoisoned(), Heap::inNursery(), Heap::inOld()

### Community 124 - "指摘"
Cohesion: 0.17
Nodes (11): 02 インタプリタ・送信・コンテキスト・プロセス, [Critical] 再帰の深さに上限がなく、C スタックが溢れてプロセスごと落ちる, [High] Process / Semaphore が実行を切り替えない（fork の本体は実行されず、wait もブロックしない）, [High] 失敗センチネル（空 OOP）がオペランドスタックに「値」として積まれ、エラーが黙って Message に化ける, [High] 非局所リターン中に Kernel の反復ネイティブが止まらない（副作用が続き、戻り値も誤る）, [Low] BlockContext をクロージャとアクティベーションに兼用し、`sender` を上書きしたまま戻さない, [Low] DNU の Message 割り当てに GC リトライがなく、ナーサリ逼迫時に DNU が空 OOP になる, [Low] `valueWithArguments:` が Array 以外のポインタオブジェクトを受け入れ、内部スロットを引数に展開する (+3 more)
...
### Community 135 - "TEST"
Cohesion: 0.06
Nodes (35): AllocateNoGcSpillsToOld, ByteObjectPayload, EnvEnablesStress, EveryFourthStressCollectionAlsoCompactsOldAndPoisonsTail, ExhaustionReturnsEmpty, FreedNurseryIsPoisonedAndReallocationIsClean, GcStress, GcStressDeathTest (+27 more)

### Community 136 - "指摘"
Cohesion: 0.06
Nodes (25): 00 直近差分（origin/main..HEAD の 6 コミット）, [Critical] ソース未保存メソッドのプレースホルダを Accept すると本体が消える, [Critical] 既存クラスの定義を Accept すると全メソッドが消える, [Critical] 自分を含む Array の `=` でスタックオーバーフロー, [High] ClassMethodCache の無効化が定義クラスの分だけ, [High] out == NULL の Do it が副作用ありで AO_ERR を返す, [High] `| q |` の宣言 temp が Workspace 束縛と混ざる, [High] `Smalltalk` が knownGlobals に含まれない (+17 more)
...
- **Why does `Oop` connect `Oop` to `WellKnown`, `.fromSmallInteger`, `LargeInteger.cpp`, `TEST`, `send2`, `Object.cpp`, `Stream.cpp`, `Float.cpp`, `Array.cpp`, `Heap.cpp`, `TEST`, `assemble`, `ImageSelector`, `unwinding`, `Roots.cpp`, `stubA`, `BlockProbe`, `Heap`, `CompileEnv`, `Roots`, `Geometry.cpp`, `intern`, `ao_image_load`, `Boot`, `TEST`, `TEST`, `Session.cpp`, `abortingSubclass`, `native_send_test.cpp`, `WellKnown::checkSmallIntegerFastPath`, `Gc`, `NativeMethod.cpp`, `TEST`, `handlescope`, `TEST`, `string`, `imageRegistryStubA`, `answerOne`, `P2-02 class object skeletons`, `ImageSave.cpp`, `abortingNew`, `TEST`, `CallContext`, `.nil`, `TEST`, `BlockContext.cpp`, `TEST_F`, `TEST`, `SmallInteger.cpp`, `.isHeap`, `String.cpp`, `Compile.cpp`, `allocateRetry`, `CompileError`, `WellKnown.cpp`, `TEST`, `Boolean.cpp`, `abi.cpp`, `.false_`, `TEST`, `TEST`, `ClassPool.cpp`, `GarbageFirstBoot`, `README.md`, `ao_Magnitude_lessOrEqual`, `Bootstrap.cpp`, `TEST`, `native_method_test.cpp`, `P6b vendor file-in Implementation Plan`, `ImageLoad.cpp`, `vector`, `RootedArray`, `Character.cpp`, `.heapPointer`, `Interpreter.cpp`, `ImageHeader`?**
  _High betweenness centrality (0.378) - this node is a cross-community bridge._
- **Why does `vector` connect `vector` to `Codegen.cpp`, `LargeInteger.cpp`, `TEST`, `Scanner.cpp`, `VendorExtract.cpp`, `Object.cpp`, `Emitter`, `Literal`, `ao_main.cpp`, `ChunkAction`, `CompileEnv`, `Roots`, `Session.cpp`, `Gc`, `NativeMethod.cpp`, `handlescope`, `ChunkParser.cpp`, `string`, `applyClassDef`, `ImageSave.cpp`, `Ast`, `Parser`, `.isHeap`, `Compile.cpp`, `allocateRetry`, `CompileError`, `TEST`, `.false_`, `TEST`, `TEST`, `ClassPool.cpp`, `Bootstrap.cpp`, `ImageLoad.cpp`, `Vendor.hpp`?**
  _High betweenness centrality (0.148) - this node is a cross-community bridge._
- **Why does `WellKnown` connect `WellKnown` to `Oop`, `Stream.cpp`, `P2-02 class object skeletons`, `WellKnown.cpp`, `Heap`, `Roots`, `intern`, `.false_`, `uint64_t`, `WellKnown::checkSmallIntegerFastPath`?**
