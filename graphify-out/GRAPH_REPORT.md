...
## Corpus Check
- 243 files · ~113,992 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 77 file(s) not represented in the graph (top: .st 62, (none) 13, .toml 1)

## Summary
- 3325 nodes · 8577 edges · 172 communities (146 shown, 26 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 1103 edges (avg confidence: 0.87)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9f55c48a`
...
### Community 1 - "Interpreter::run"
Cohesion: 0.22
Nodes (17): safepoint, allocateRetry(), byteCount(), CallContext, int16_t, uint16_t, uint8_t, decodeHeader() (+9 more)

### Community 2 - ".isHeap"
Cohesion: 0.11
Nodes (66): [High] Dictionary / Set が hash を捨てて線形探索し、要素数の 2 乗で遅くなる, ネイティブのルートを構造で保証する, ネイティブのルートを構造で保証する, int64_t, Gc::clearWeakAfterOldMark(), allocateRetry(), ao_Association_key(), ao_Association_key_value_() (+58 more)
...
### Community 15 - "Bootstrap"
Cohesion: 0.13
Nodes (18): AddInternsByFunctionPointer, AtPutFindsInternedKey, Bootstrap, GrowKeepsOuterOopAndEntries, install, MethodDictionary, NameAndApply, TEST() (+10 more)
...
### Community 20 - "Roots"
Cohesion: 0.08
Nodes (20): StackWalker, uint32_t, uint8_t, vector, Roots, dropHandle, free_, handles_ (+12 more)
...
Cohesion: 0.17
Nodes (27): Task 1: 形式ヘッダと即値ビット, allocateRetry(), ao_String_asSymbol(), ao_String_at_(), ao_String_at_put_(), ao_String_equals(), ao_String_printString(), ao_String_size() (+19 more)
...
Cohesion: 0.05
Nodes (100): Task 10: accept, Image, load, save, boot(), blankOut(), browserClassAt(), browserClassCount() (+92 more)
...
Cohesion: 0.13
Nodes (22): climits, dyld, fstream, interpreter, iterator, addRoots, bootAndRunTests(), string (+14 more)
...
### Community 35 - "gtest"
Cohesion: 0.13
Nodes (15): BoxFromImageNativeCodeNil, BoxLiteralArrayPseudoObjects, chrono, Chunk, compile, CompiledMethod, compiler, context (+7 more)

### Community 36 - "ChunkParser.cpp"
Cohesion: 0.19
Nodes (24): atLineEnd(), bangSpaceBangAt(), classify(), string, string_view, uint32_t, vector, firstLine() (+16 more)
...
Cohesion: 0.11
Nodes (31): CompileError, message, span, string, [Low] チャンクの種別を 1 行目のキーワードで判定するので、メソッドをクラス定義と誤認する, P6b vendor file-in Implementation Plan, Self-review, Task 3: rebind と上書き禁止 (+23 more)
...
Cohesion: 0.15
Nodes (28): [High] イメージ保存が非アトミックで、書き込みに失敗すると既存イメージが壊れる, Gc::clearWeakAfterNursery(), Gc::copy(), appendRaw(), appendRecord(), collectExtra(), collectImageSlot(), byte (+20 more)
...
### Community 62 - "TEST"
Cohesion: 0.15
Nodes (12): AbiSmoke, BootAndShutdownReturnZero, BootVersionShutdown, 06 イメージ・セッション・C ABI・CLI, [Low] `ao_version` が切り詰め時に AO_ERR_RANGE ではなく AO_ERR を返す, テストの穴, 概要, 結論 (+4 more)
...
Cohesion: 0.07
Nodes (33): string_view, isVendorStub(), deque, size_t, string, string_view, unordered_map, Entry (+25 more)
...
Cohesion: 0.09
Nodes (35): AcceptAbi, AoTranscriptFn, ClassDefinitionThenImageDropsSourceText, [Low] count 系 ABI がエラーで AO_ERR(=1) を返し、件数 1 と区別できない, [Low] transcript フックが boot 前、または shutdown→boot 後に配線されない, Browser の読み取り, Global Constraints, Task 11: Workspace の Do it / Print it / Inspect it (+27 more)

### Community 88 - "claude-review/README.md"
Cohesion: 0.14
Nodes (11): 04 Object・Behavior・コレクション系 Kernel ネイティブ, テストの穴, 既報との関係（件数に含めない）, 概要, Ao, ビルドとテスト, ライセンス, 必要環境 (+3 more)
...
### Community 92 - "Gc"
Cohesion: 0.10
Nodes (28): algorithm, Gc, clearWeakAfterNursery, clearWeakAfterOldMark, collectOld, copy, failed_, heap_ (+20 more)

### Community 95 - "collectNursery"
Cohesion: 0.28
Nodes (15): [Medium] メソッド辞書の拡張に失敗するとメソッドを黙って捨て、`installMethod` は成功を返す, [High] チャンク経由でメソッドを再定義してもメソッドキャッシュを無効化しない, collectNursery, acceptMethodSource(), allocateRetry(), boxBytes(), boxLiteral(), boxMethodImage() (+7 more)
...
Cohesion: 0.15
Nodes (11): ao_abi, cstdio, cstring, optional, runtime, ao executable, ao_runtime library, ao_runtime_tests (+3 more)
...
Cohesion: 0.17
Nodes (36): decodeNonHeap, acceptWord(), bindAll(), checkGlobals(), byte, size_t, string, string_view (+28 more)
...
### Community 122 - "Oop"
Cohesion: 0.14
Nodes (13): bits(), int64_t, isBytes(), isIndexable(), isPointers(), make(), uint64_t, Oop (+5 more)

### Community 123 - "TEST_F"
Cohesion: 0.14
Nodes (13): BlockAssignmentDoesNotUpdateWorkspaceBinding, BootThenImageRoundTripKeepsOnePlusTwo, EvalTranscriptShowThenCr, InspectItPrintsAndNotifiesHook, KnownGlobalStaysGlobal, MissingLoadOrderIsError, PrintItInsertsThree, testing::Test (+5 more)

### Community 124 - "ao_accept_class"
Cohesion: 0.15
Nodes (19): 02 インタプリタ・送信・コンテキスト・プロセス, 概要, 03 数値・スカラー系 Kernel ネイティブ, テストの穴, 概要, 05 コンパイラ（スキャナ・パーサ・コード生成・チャンク）, テストの穴, 概要 (+11 more)
...
Cohesion: 0.12
Nodes (15): cstddef, HeaderRoundTripAndRejects, HeapShapedBitsAreNotImmediates, ImmediateBitsRoundTrip, ImageFormat, encodeNonHeap, kImageEndianLittle, kImageFillerBytes (+7 more)
...
Cohesion: 0.19
Nodes (5): string, vector, filesystem, MethodImage, sstream

### Community 133 - "指摘"
Cohesion: 0.13
Nodes (20): CompileEnv, instVarNames, knownGlobals, undeclaredAreTemps, workspaceTemps, [Critical] Kernel クラスのサブクラスで、インスタンス変数の添字が親の物理スロットと重なる, [High] 2^63 以上の整数リテラルが黙って 0 になる, [High] クラス変数を読み書きできない (+12 more)

### Community 134 - "ao_eval"
Cohesion: 0.12
Nodes (19): [High] Workspace 束縛が 255 temp の上限に達すると eval が全滅, 07 Swift アプリ・ビルド・スクリプト, [Low] `AoSpan` の区間を捨てているので、エラー位置が分からない, [Low] C++ のデプロイメントターゲットがアプリの最小 OS と一致していない, [Low] .gitignore に `.cache/` と `.serena/logs/` が無い, [Low] Inspector ウィンドウが閉じても解放されず、増え続ける, [Low] package-app.sh が作るバンドルは署名検証に通らない Debug ビルドになる, [Low] Print it の結果が NUL を含むと、そこで途切れて壊れた文字列が挿入される (+11 more)

### Community 135 - "TEST"
Cohesion: 0.12
Nodes (18): EnvEnablesStress, EveryFourthStressCollectionAlsoCompactsOldAndPoisonsTail, FreedNurseryIsPoisonedAndReallocationIsClean, GcStress, GcStressDeathTest, InternSameBytesIsIdentical, InternSurvivesNurseryGc, inNursery (+10 more)

### Community 136 - "指摘"
Cohesion: 0.12
Nodes (15): 00 直近差分（origin/main..HEAD の 6 コミット）, [Critical] クラス定義でない文字列が AO_OK で黙って捨てられる, [Critical] ソース未保存メソッドのプレースホルダを Accept すると本体が消える, [Critical] 既存クラスの定義を Accept すると全メソッドが消える, [Critical] 自分を含む Array の `=` でスタックオーバーフロー, [Critical] 非クラス名で ao_accept_method を呼ぶと範囲外書き込み, [High] ClassMethodCache の無効化が定義クラスの分だけ, [High] out == NULL の Do it が副作用ありで AO_ERR を返す (+7 more)
...
### Community 156 - "HostTempGuard"
Cohesion: 0.25
Nodes (8): HostTempGuard, active, count, ctx, prevCount, prevPtr, ptr, temps

### Community 157 - "P1-07: immovable old objects"
Cohesion: 0.29
Nodes (6): Files, Interfaces, P1-07: immovable old objects, テスト, 手順, 目的

### Community 158 - "P4-08 Point / Rectangle"
Cohesion: 0.33
Nodes (6): Bootstrap, NativeMethod, Point, P4-08 Point / Rectangle, Rectangle, send

### Community 159 - "applyMethod"
Cohesion: 0.50
Nodes (5): GC 安全性（ネイティブ）の横断評価 — 03 からの照会への回答, テストの穴, Interpreter, run, applyMethod()

### Community 160 - "ActiveGuard"
Cohesion: 0.50
...
## Knowledge Gaps
- **739 isolated node(s):** `.metaFlag`, `.title`, `.sourceText`, `.errorText`, `.paneAccessibilityLabels` (+734 more)
...
- **Why does `Oop` connect `Oop` to `WellKnown`, `Interpreter::run`, `.isHeap`, `LargeInteger.cpp`, `image_save_load_test.cpp`, `Object.cpp`, `Stream.cpp`, `Float.cpp`, `Heap.cpp`, `putNative`, `Bootstrap`, `指摘`, `send`, `Heap`, `Roots`, `Geometry.cpp`, `SmallInteger.cpp`, `String.cpp`, `Send.cpp`, `Session.cpp`, `TEST`, `TEST`, `gtest`, `P3 — Native Dispatch`, `ImageSave.cpp`, `CallContext`, `ClassDef`, `BlockContext.cpp`, `TEST`, `.nil`, `Boot`, `指摘`, `DiskHeader`, `WellKnown.cpp`, `TEST`, `send2`, `cstdint`, `intern`, `.false_`, `abi.cpp`, `claude-review/README.md`, `Globals.cpp`, `allocateRetry`, `Gc`, `collectNursery`, `Character.cpp`, `Bootstrap.cpp`, `CompiledMethodNatives.cpp`, `cstring`, `ImageLoad.cpp`, `Magnitude.cpp`, `Boolean.cpp`, `imageRegistryStubA`, `NameCollect`, `ObjectHeader`, `ImageFormat`, `string`, `指摘`, `Array.cpp`, `uint32_t`, `performSend`, `Roots.cpp`, `ClassMethodCache`, `Interpreter.cpp`, `gc_test.cpp`, `Frame`, `HostTempGuard`, `applyMethod`, `ActiveGuard`, `Root`, `Root`, `Root`, `Root`, `Root`, `Root`, `Root`, `Root`, `Root`, `Root`?**
  _High betweenness centrality (0.360) - this node is a cross-community bridge._
- **Why does `Heap` connect `Heap` to `WellKnown`, `Claude レビュー指摘の修正計画（docs/claude-review）`, `.isHeap`, `LargeInteger.cpp`, `image_save_load_test.cpp`, `TEST`, `Object.cpp`, `Float.cpp`, `Array.cpp`, `Heap.cpp`, `P7 .aoimage Implementation Plan`, `01 オブジェクトメモリ・GC・ブートストラップ`, `TEST`, `P8–P9 Remaining Implementation Plan`, `putNative`, `Stream.cpp`, `send`, `gc_test.cpp`, `Roots`, `Bootstrap`, `TEST`, `P1 — Object Memory`, `String.cpp`, `Send.cpp`, `Session.cpp`, `TEST`, `ao_main.cpp`, `gtest`, `P3 — Native Dispatch`, `ImageSave.cpp`, `CallContext`, `BlockContext.cpp`, `.nil`, `Boot`, `指摘`, `DiskHeader`, `WellKnown.cpp`, `TEST`, `cstdint`, `intern`, `Globals.cpp`, `Gc`, `Bootstrap.cpp`, `Claude レビュー指摘の修正計画（docs/claude-review）`, `ao_image_load`, `cstring`, `ImageLoad.cpp`, `ObjectHeader`?**
  _High betweenness centrality (0.142) - this node is a cross-community bridge._
- **Why does `WellKnown` connect `WellKnown` to `Heap`, `Oop`, `cstdint`, `Roots`?**
...
- **Are the 70 inferred relationships involving `send()` (e.g. with `GC 安全性（ネイティブ）の横断評価 — 03 からの照会への回答` and `[Low] `Roots` が同じスロットの二重登録を許し、`collectOld` の更新は冪等でない`) actually correct?**
  _`send()` has 70 INFERRED edges - model-reasoned connections that need verification._