...
- full GC はスキャベンジの後、`oldUsed > threshold` のときだけ走らせる。`threshold = max(初期容量, 2×生存量)`。例外は次の 1 つだけ: GC を走らせてよい割り当て（`allocateRetry`）が old の上限で失敗したときは、full GC を 1 回走らせてから割り当てを 1 回やり直す。GC を走らせない割り当て（メソッド辞書の作成・拡張、Symbol の intern）は、上限でそのまま失敗する。
