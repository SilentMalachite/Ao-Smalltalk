...
- `Class>>classPool` はその辞書を答える。`Foo classPool at: #Count` は束縛（Association）で、その `value` がクラス変数の値である。
...
- 名前の解決順は、ローカル（メソッドとブロックの引数と temp）→ インスタンス変数 → 擬変数 → クラス変数（§3.6）→ グローバル（`Smalltalk` の辞書。§3.6）である。前のものが、後ろの同じ名前を隠す。ワークスペース（§3.10）の解決順は §3.10 に書く。ワークスペースのレシーバは nil なので、クラス変数は無い。
  ...
  - クラス側のメソッドでは、Behavior の 8 つの枠の名前（`superclass methodDict format name thisClass category classPool instVarNames`。§3.6）がインスタンス変数なので、同じ名前のクラス変数を隠す。
...
- 形が同じとき（superclass が、名前で引いた同じクラスで、instVarNames が同じ名前の同じ順のとき）は、既存のクラスオブジェクトを保つ。メソッド辞書（インスタンス側とクラス側）、メタクラス、既存インスタンスはそのままである。更新するのは category と classVariableNames（classPool。§3.6）だけである。classPool は次のように更新する。
  ...
  - 消える名前の束縛を、そのクラスかそのサブクラス（子孫すべて）のメソッドが持っていれば、何も変えずに `AO_ERR_COMPILE` を返す（category も変えない）。インスタンス側とクラス側のメソッド、ソースの無いメソッド、ブロックの中を数え、読みも代入も数える。束縛を持つメソッドは、classPool から外れた束縛を黙って読み書きし続けるからである。メッセージは `class variable change refused: <Class>>><selector> refers to removed class variable <var>` で、`<Class>` はメソッドのあるクラス（クラス側なら `<Class> class`）である。どのメソッドも持たなければ、消える名前を classPool から除く。
...
- 形が変わるとき、次のどれかに当たれば、何も変えずに `AO_ERR_COMPILE` を返す。名前は旧クラスを指したまま、旧クラスとそのメソッドもそのままである。`AoSpan.message` は空にせず、理由（当たったセレクタ、サブクラスがあること、または消えるインスタンス変数かクラス変数）を入れる。
  ...
  - インスタンス側かクラス側のメソッドが、旧クラスから見えて新しい定義から見えないクラス変数（§3.6。スーパークラスのものを含む）を名前で読む。インスタンス変数と同じ理由で、数え方も同じである（代入は `cannot assign` でコンパイルし直しの失敗になる）。メッセージは `shape change refused: <Name>>><selector> refers to removed class variable <var>`（クラス側は `<Name> class>><selector>`）である。消えるインスタンス変数の読みがあれば、そちらを先に報告する。
